...
<div class="form-group has-feedback">
					...
					<table id="itemdetailslist" class="table table-bordered table-striped dTable">
						<thead style="background:#99ceff;">
							<tr>
								...
								<th style="border-bottom:0px; width:10%">Bill Date</th>
								<th style="border-bottom:0px; width:9%">Amount</th>
								<th style="border-bottom:0px; width:13%">Admission No.</th>
	...
	<!--							<th style="border-bottom:0px; width:9%">Approver1</th>
								...
								<th style="border-bottom:0px; width:16%">Status</th>
									...
									<td style="text-align:center;">
										...
										if(substr($right_access,2,1)=='U') {?>
											<a class="btn a-edit" style="color:red" onclick="updateItemDet('<?php echo $arritem->id; ?>','<?php echo $arritem->bill_date; ?>', 
																					...
																					'<?php echo $arritem->approve4_comment; ?>', '<?php echo $arritem->date_approved4; ?>',
																					'<?php echo $get_role; ?>'
																					);">
...
function updateItemDet(id, bill_date, student_enrol_id, bill_amount, full_name, approve1_comment, app1date, approve2_comment, app2date,approve3_comment, app3date,approve4_comment, app4date, gtype) {
		$('#errmodal').empty();
...
//				$('#date_approve1').val(date('Y-m-d'));
		...
		else if (gtype == 'admin' || gtype == 'Approver 4') {
			if (($('#approve4_comment').val() != null)) {
				$('#approve4_comment').attr('disabled',true);
			}
			else {
				$('#approve4_comment').attr('disabled',false);
				$('#remarks').attr('disabled',false);
			}
		}		
...
function save() {
		...
		if (resp == true) 
		{
				...
				if (($('#approve1_comment').val() == 'Rejected') || ($('#approve2_comment').val() == 'Rejected') || ($('#approve3_comment').val() == 'Rejected')  || ($('#approve4_comment').val() == 'Rejected')) {
...
//	alert($('#approve1_comment').attr('disabled'));
				...
				if(err == '') {
					...
					var comm_appr4 = $('#approve4_comment').val();
					var date_appr4 = $('#date_approve4').val();
								...
								data: 
									{
										...
										comm_appr4:comm_appr4,
										date_appr4:date_appr4
									},
								dataType: "text",
...
<!--    **  Modal content    **  
						...
						<div class="form-group well">
							<div class="col-md-8 nogap">
								...
								<div class="form-group nogap">
									<label class="control-label col-sm-3 nogap">Billclerk :</label>
									<div class="col-sm-3 nogap">
								...
								<div class="form-group nogap">
									...
									<div class="col-sm-3 nogap">
										<select class="form-control" name="approve3_comment" id="approve3_comment" value="" disabled>  <!--onchange="updDate(id, value);"  -->
											<option value="None">&nbsp;</option>
											<option value="Approved">Approved</option>
											<option value="Rejected">Rejected</option>
										</select>
								...
								<div class="form-group nogap">
									...
									<div class="col-sm-3 nogap">
										<select class="form-control" name="approve4_comment" id="approve4_comment" value="" >
											<option value="None">&nbsp;</option>
											<option value="Approved">Approved</option>
											<option value="Rejected">Rejected</option>
										</select>
									...
									<label class="control-label col-sm-2 nogap">Date :</label>
									<div class="col-sm-4 nogap">
										<input type="date" class="form-control" id="date_approve4" name="date_approve4" value="" disabled>
									</div>
								</div>
								<!-- <div class="form-group nogap"> -->
							...
							<div class="col-md-4 nogap">
								...
								<div class="col-md-12 nogap">
									<textarea rows="3" class="form-control" id="remarks" name="remarks" value="" placeholder="Required, if rejected..." ></textarea>
								</div>
							</div>
						</div>
					</div>
