        ...
        <table id="studentlist3" class="table table-bordered table-striped">
            <thead>
                <tr> 
					...
					{?><th rowspan="2" style="border-bottom:1px solid black;"> FEE CATEGORY </th><?php }?>
					...
					<?php if(!is_array($rmvcol) || !in_array('status', $rmvcol)) 
					{?><th rowspan="2" style="border-bottom:1px solid black;"> STATUS </th><?php }?>
			...
			<tbody id="monthlyfees_load_td">
                ...
                ?>                  
					<tr>
						...
						<?php if(!is_array($rmvcol) || !in_array('fee_cat', $rmvcol)) 
						{?><td><?php if($schgrp=='ARMY') {echo 'Quarterly Fee';}else{echo 'Monthly Fee';}?></td><?php }?>
						...
						<?php if(!is_array($rmvcol) || !in_array('status', $rmvcol)) 
						{?><td><?php echo $ftd->response_message;?></td><?php }?>
						...
						<td><?php 
							$m=array();
							$m=explode(',',$ftd->month_details);$str='';
							foreach($m as $v) { 
								$str.=$month_arr[$v].' ';
								} 
								echo $str;?>
						...
						<?php $total=0;foreach($fee_ty as $d=>$v) {		// *$fee_ty*  is for **No. of FEE_TYPES**
							$fee_amt=0;
							...
							if(!is_array($rmvcol1) || !in_array($d, $rmvcol1)) {
								...
								if ($fee_cat_id[$d]!=8 && count($class_fee) > 0) {
									...
									if (count($fee_qry) > 0) {
										if($fee_qry[0]->month_no!=0){
											if($fee_cat_id[$d] == 5) {
												$de = ($class_fee[0]->fee_amount)/3;	//** AMITABH 12-Oct-2020 :: Removed ROUND function *
											}
											else {
												$de = round($class_fee[0]->fee_amount); //round($class_fee[0]->fee_amount);
											}
										}
										else {
											if($fee_cat_id[$d]==1) {
												...
												if($fee_id==10 || $fee_id==16) {
													$de = round($fee_amount);  //Added round
												}                
												else {
													// *AMITABH 12-Oct-2020 : Added to escape fractions with 0.5
													$paise_fifty = ($fee_amount/12)*$rest_month;
													if ((round($paise_fifty) - $paise_fifty) == 0.5) {
														$de = $paise_fifty;
													} else{
														$de = round($paise_fifty);
													} 
													// *AMITABH :: Code ends here
												}
											}
											else {
												//$dee	= ($class_fee[0]->fee_amount/12);
												//$de		= round($dee * 12); // AMITABH : Added **round**  // Need to check if this code is needed
										...
										foreach ($fee_qry as $rfee) {
											$fee_amt = $fee_amt + $de; //AMITABH added round 
											$total  += $de; //amitabh
										}
									...
									if (count($fee_qry) > 0) {
										foreach ($fee_qry as $rfee) {
											$fee_amt = round($fee_amt + $rfee->amount); //AMITABH added round 
											$total += round($rfee->amount); //AMITABH added round 
										}
								...
								// *AMITABH 12-Oct-2020 : Added to escape fractions with 0.5
								$paise_fifty = $total_fee_amt_wise[$d]+$fee_amt;
								if ((round($paise_fifty) - $paise_fifty) == 0.5) {
									$total_fee_amt_wise[$d] = $paise_fifty; //(($fee_amount/12)*$rest_month);
								}else{
									$total_fee_amt_wise[$d] = round($paise_fifty); //(($fee_amount/12)*$rest_month); // * AMITABH : put this is ELSE condition
								}
								// AMITABH
								
							//	$total_fee_amt_wise[$d]=($total_fee_amt_wise[$d]+$fee_amt);  // *AMITABH - Rounding not needed here *
								?>
					...
					<tr>
						...
						<?php 
						foreach($fee_ty as $d=>$v) {	// *For Each FEE_TYPE
                      // print_r($v);
                      // echo '-----';
