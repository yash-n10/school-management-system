...
//                          echo $fee_cat_id[$d].'<br>';
                          $fee_amt=0;
                          if(!is_array($rmvcol1) || !in_array($d, $rmvcol1)) {
//                          $max_class_year = $this->dbconnection->select('class_fee_head', 'max(year) as max_year, max(id) as max_id', "(from_class_id<=$stud_class and to_class_id>=$stud_class) and course=$stud_course and status='Y' and year<=$session_start_yr");
