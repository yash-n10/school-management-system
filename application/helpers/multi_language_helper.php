...
if ( ! function_exists('get_phrase'))
{
	function get_phrase($phrase = '') {
		...
		$query	=	$CI->db->get_where('crmfeesclub.language' , array('phrase' => $phrase));
		$row   	=	$query->row();
		
		/*** insert blank phrases initially and populating the language db ****/
		$check_phrase	=	$CI->db->get_where('crmfeesclub.language' , array('phrase' => $phrase))->row()->phrase;
		if ( $check_phrase	!=		$phrase)
			$CI->db->insert('crmfeesclub.language' , array('phrase' => $phrase));
