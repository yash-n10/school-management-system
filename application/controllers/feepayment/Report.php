    ...
    public function fee_summary() {
     ...
     $schoolgrp=$this->dbconnection->Get_namme("crmfeesclub.school","id",$this->id,"school_group");
        ...
        if($schoolgrp=='ARMY')
        {
            $this->data['page_title'] = 'Fee Summary';
            $this->data['section'] = 'feepayment/report';
            $this->data['page_name'] = 'fee_summary_army2';
            $this->data['customview'] = '';
            $this->data['school_date_created'] =$this->school_date_created;
            $this->data['collection_center'] = $this->dbconnection->select("collection_center", "id,collection_code,collection_desc", "status='Y'");
            $fee_type = $this->dbconnection->select_returnarray("fee_master", "id,fee_name,fee_cat_id", "status=1");
            ...
            $this->data['page_title'] = 'Fee Summary';
            $this->data['section'] = 'feepayment/report';
            $this->data['page_name'] = 'fee_summary2';
            $this->data['customview'] = '';
            $this->data['school_date_created'] =$this->school_date_created;
            $this->data['session'] =$this->dbconnection->select("accedemic_session", "*", "status='Y'","id","DESC");
            $this->data['collection_center'] = $this->dbconnection->select("collection_center", "id,collection_code,collection_desc", "status='Y'");
...
//        $fetch_transaction_date = $this->dbconnection->select("fee_transaction_head", "student_id as stud, payment_date", "payment_date>=DATE_FORMAT('$fdate', '%Y-%m-%d 00:00:00') and payment_date<=DATE_FORMAT('$tdate', '%Y-%m-%d 23:59:59') and paid_status=1 and response_code=0 $str_query1 ");
        ...
        if($this->session->userdata('school_id')==25){


        $this->load->view('feepayment/report/load_monthlyfee_wise_fee_summ_davh', $fee_details);
}
else{
    ...
    public function monthlyfees_wise_fee_summary2() {
        // print_r($this->input->post());echo "<br><br><br><br>";
        $aca_session = $this->input->post('aca_session');
        $fdate = $this->input->post('frmdate');
        $ftime=' 00:00:00';
        $fdate=$fdate.$ftime;
        ...
        $ttime=' 23:59:59';
        $tdate=$tdate.$ttime;
        $collection=$this->input->post('collection_center');
        if ($this->input->post('collection_center') == 'all') {
            $str_query = '';
            // $str_query1 = '';
        } else {
            $str_query = "and collection_centre= '$collection'";

            // $str_query1 = " and fee_transaction_head.collection_centre= '{$this->input->post('colcenter')}'";
        }

        // $str_query = "fh.payment_date>=DATE_FORMAT('$fdate', '%Y-%m-%d 00:00:00') and fh.payment_date<=DATE_FORMAT('$tdate', '%Y-%m-%d 23:59:59')"; 
        $query="SELECT * FROM fee_transaction_head WHERE payment_date >= '$fdate' and payment_date <= '$tdate' and year='$aca_session' $str_query and paid_status=1";
        // print_r($query);die();
        $fee_details=array();
        $fee_transaction_head=$this->db->query("SELECT * FROM fee_transaction_head WHERE payment_date >= '$fdate' and payment_date <= '$tdate' and year='$aca_session' and paid_status=1 $str_query");
        $fee_transaction_head=$fee_transaction_head->result();
        // echo "<pre>";print_r($fee_transaction_head);
        if($this->session->userdata('school_id')==29){
           $this->data['page_title'] = 'FEE SUMMARY TYPE 3';
        $this->data['section'] = 'feepayment/report';
        $this->data['page_name'] = 'dcr_yash';
        $this->data['fee_transaction_head']=$fee_transaction_head;
        $this->load->view('index', $this->data);
        }
...
//            $recarr[] = $row->father_name;
    ...
    public function book_list_report()
    {
        ...
        $this->data['customview'] = '';
        $this->data['school_date_created'] =$this->school_date_created;
        $this->data['book_stock_report']=$this->db->query('select bsm.fee_id,(select fee_name from fee_master where id=bsm.fee_id) fee_name,bsm.opening_qty,bsm.sale_qty,bsm.rest_qty,bsm.class_id,(select class_name from class where id=bsm.class_id) class_name from book_stock_master bsm where status=1 and id <103;')->result();
        $this->load->view('index', $this->data);
    }
