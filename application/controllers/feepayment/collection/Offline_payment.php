...
//            redirect(base_url(), 'refresh');
    ...
    public function load_student_fee_div() {
       ...
       // $this->db->db_debug=TRUE;
        ...
        // $stud_acedemic_session_id = $this->input->post('fee_session');
        // $stud_academic_session_id=2;
       ...
       // print_r($active_);die();
        if($active_=='U')
        {
            $fee_session_query = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$class and to_class_id>=$class) and course=" . $course_id . " and status='Y' and year=".$stud_acedemic_session_id);
        }
        else if($active_=='Y')
        {
            $fee_session_query = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$class and to_class_id>=$class) and course=" . $course_id . " and status='Y' and year=".$stud_acedemic_session_id);
        ...
        {?>
            <script>
                alert('Please Create Fees Structure');
                window.location.assign("https://erp.feesclub.com/feepayment/class_fee");
            </script>
...
//            if (count($fetch_fees_head) > 0) {
        ...
        if ($max_class_fee_id != NULL || $max_class_fee_id != '') {
            ...
            // echo "<pre>";print_r($onetime_fee->reult());die();
            $onetime_fee_paid = $this->db->query('SELECT * from fee_transaction_det where fee_trans_head_id in( select id from fee_transaction_head where year=' . $stud_acedemic_session_id . ' and student_id=' . $fetch_stud[0]->id . ' and paid_status=1 and status=1) and fee_cat_id in (9,10)')->result();
            /* ------------------------------------------------------------------------------------ */
            ...
            $qpm = $this->dbconnection->select("fee_transaction_det", "count(month_no) cnt_paid", "fee_trans_head_id in(select id from fee_transaction_head where year=$stud_acedemic_session_id and student_id=" . $fetch_stud[0]->id . " and paid_status=1 and response_code=0 and status=1) and fee_cat_id=$this->fee_cat1");
            // echo "<pre>";print_r($qpm);die();
            $paid_month = $qpm[0]->cnt_paid;
            
            $chqqpm = $this->dbconnection->select("fee_transaction_det", "count(month_no) cnt_paid", "fee_trans_head_id in(select id from fee_transaction_head where year=$stud_acedemic_session_id and student_id=" . $fetch_stud[0]->id . " and paid_status=2 and response_code=0 and status=1) and fee_cat_id=2");
            ...
            $half_ann_fee_paid = $this->dbconnection->select_join('fee_transaction_head a', 'a.paid_status,a.remarks,a.receipt_no', "a.year=$stud_acedemic_session_id and a.student_id=" . $fetch_stud[0]->id . " and a.paid_status=1 and a.status=1 and b.fee_cat_id=" . $this->fee_cat2, "fee_transaction_det b", " a.id=b.fee_trans_head_id", "inner");
            ...
            $fetch_other_fees_det = $this->dbconnection->select("class_fee_det as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc,(Select month_set from fee_master where id=fee.fee_id) as month_set", "fee.class_fee_head_id=" . $max_class_fee_id . " and fee_cat=3 and fee_amount!='0' and status=1");
             $other_fee_paid = $this->dbconnection->select_join('fee_transaction_head a', 'a.paid_status,a.remarks,a.receipt_no,b.other_fee_id', "a.year=$stud_acedemic_session_id and a.student_id=" . $fetch_stud[0]->id . " and a.paid_status=1 and a.status=1 and b.fee_cat_id=3", "fee_transaction_det b", " a.id=b.fee_trans_head_id", "inner");
            ...
            //RUCHI'S CODE COMMENTED BY YASH ON 29/01/2021 FOR TESTING FEE COLLECTION OF ALL SESSIONS . 
			// if(($this->session->userdata('school_id')==29)&&($stud_acedemic_session_id==2))
   //          {
   //          	$fetch_instant_fees_det = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $stud_id . " and year=3 and paid_status!=1 and status='Y'");
   //          	$fetch_instant_fees_det1 = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $stud_id . " and year=3 and status='Y'");
   //          }
            // else
            // {
				$fetch_instant_fees_det = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $stud_id . " and year=". $stud_acedemic_session_id . " and paid_status!=1 and status='Y'");
            	$fetch_instant_fees_det1 = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $stud_id . " and year=". $stud_acedemic_session_id . " and status='Y'");
            // }
        ...
        $transaction_history = $this->db->query("select f1.*,group_concat(distinct(f2.fee_cat_id)) as fee,"
                ...
                . " max(f2.due_month_no) as d from fee_transaction_head f1, fee_transaction_det f2 where f1.student_id=" .$stud_id. " and f1.status=1 and f1.response_message is not NULL and f1.id=f2.fee_trans_head_id group by f1.id order by f1.date_created desc");
...
//            $accedmic_sssion = $this->dbconnection->select('accedemic_session', 'max(id) as max_year',"active='Y' and status='Y'");
        ...
        $fetch_stud = $this->db->query("SELECT * FROM student where admission_no='$admission_no'");
        $fetch_stud=$fetch_stud->result();
        $current_year = $fetch_stud[0]->student_academicyear_id;
        $stud_acedemic_session_id = $inputall['stud_acedemic_session_id'];
...
//$inputall['fee_amt'][7]
            ...
            $description = "Offline Collection Total $final_total - AdmissionN0 $admission_no - Student ID $student_id of session $stud_acedemic_session_id";
            ...
            $paid_mon_query = $this->dbconnection->select('fee_transaction_det', 'count(month_no) as paid', 'fee_trans_head_id in( select id from fee_transaction_head where student_id=' . $student_id . ' and paid_status=1 and response_code=0 and status=1 and year=' . $stud_acedemic_session_id . ') and fee_cat_id=2');
...
//                }
    ...
    public function update_offln_payment_student_wise() {
        ...
        $data = array(
            ...
            'pos_no' => $this->input->post('pos_no_edit'),
