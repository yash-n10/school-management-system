...
class Offline_payment_army extends CI_Controller {
...
//        $this->db->db_debug=TRUE;
        ...
        $this->qtrset=explode('@',$this->school_desc[0]->month_set);
        $this->qtr=array();
        ...
        $this->page_name = 'offline_payment_upload_army';
        $this->customview = '';
    }
...
//                        $qpm=$this->dbconnection->select("fee_trans_det","sum(month_no) cnt_paid","year=$year and student_id=".$fetch_stud[0]->id ." and paid_status=1 and fee_cat_id=$this->fee_cat1");
            $paid_month = $qpm[0]->cnt_paid + ($start_fee_month-1);
...
//            $fetch_other_fees_det_month = $this->dbconnection->select("class_fee_det as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc,(Select month_set from fee_master where id=fee.fee_id) as month_set", "fee.class_fee_head_id=" . $max_class_fee_id . " and fee_cat=3 and fee_amount!='0' and status=1");
        ...
        $transaction_history = $this->db->query("select f1.*,group_concat(distinct(f2.fee_cat_id)) as fee,"
                . " count(case when f2.month_no<>0 and f2.fee_cat_id=5 then f2.month_no end) as m,min(case when f2.month_no<>0 and f2.fee_cat_id=5 then f2.month_no end) as from_month,"
...
//                        $due_month = $due_month + 1;
        ...
        if ($fine_rule[0]->cnt != 0) {
                if ($fine_rule[0]->mdue_month < $due_month) {
                    ...
                    $fine_rule_duemonth=$due_month;
                    
//                    $due_month = !empty($fine_rule_fetch) ? $fine_rule_fetch[0]->max_day : $due_month;
                }
            $fine_rule_fetch = $this->dbconnection->select_join("fine_rule fr", "fr.id,fr.from_day,fr.to_day,fr.due_month,fr.max_day,cf.fee_amount,cf.fine_rule_id", "fr.due_month=$fine_rule_duemonth and fr.from_day<=$day "
            ...
            $due_month=$fine_rule_fetch[0]->max_day;
        }
...
//            echo $fine_amount;
        ...
        $collection_center_qry = $this->dbconnection->select("collection_center", "id,collection_code,collection_desc", "status='Y' and id!=1");
        ...
        $this->load->view('feepayment/collection/load_fee_collection_army', $data);
    }
...
//        $actual_due_month = $this->input->post('fine_month');
        $actual_due_month=$original_due_month = $this->input->post('original_due_month');
        ...
        if (!empty($fine_rule_fetch)) {
            ...
            if ($this->school_desc[0]->fine_type_checkbox == 'ADJUSTABLE') {
                ...
                $fine_rule_fetch = $this->dbconnection->select_join("fine_rule fr", "fr.id,fr.from_day,fr.to_day,fr.due_month,cf.fee_amount,cf.fine_rule_id", "fr.due_month=$actual_due_month and fr.from_day<=$day "
                    . "and fr.to_day>=$day and cf.fee_cat=0 and cf.class_fee_head_id=$max_class_fee_id and cf.status=1 and cf.stud_cat=0","class_fee_det cf","fr.id=cf.fine_rule_id","inner");            
                $fine_amount=$fine_rule_fetch[0]->fee_amount;
...
//        $fee_session_year=$this->input->post('fee_session_year');
    ...
    //                    $half_yearly_month= array_column($half_yearly_fee->result_array(), 'month_set','id');
        ...
        for ($i = $paid_month+1; $i <= $paid_month+$noofmonth; $i++) {
                    ...
                    $dbq = $this->db->query("SELECT id,fee_name,month_set,(SELECT fee_amount FROM class_fee_det where fee_cat=4 and stud_cat=$category_id and class_fee_head_id=$class_fee_head_id and status=1 and fee_id=fee_master.id) fee_amount FROM `fee_master` WHERE fee_cat_id=4 and FIND_IN_SET('$i', month_set) group by id")->result();
//                    print_r($dbq);
...
//$inputall['fee_amt'][7]
            ...
            'total_amount' => $final_total, 'discount_amount' => $inputall['fee_amt'][7], 'paid_by' => $this->session->userdata('user_id'), 
            'payment_date' => date('Y-m-d H:i:s', strtotime($payment_date)),'remarks' => $remarks, 'mode' => $inputall['mode_payment'], 
            'cheque_no'=>$inputall['cheque_no'],'cheque_date'=>$inputall['cheque_date'],'cheque_status'=>$inputall['cheque_status'],
            ...
            $paid_mon_query = $this->dbconnection->select('fee_transaction_det', 'count(month_no) as paid', 'fee_trans_head_id in( select id from fee_transaction_head where student_id=' . $student_id . ' and paid_status=1 and response_code=0 and status=1 and year=' . $current_year . ') and fee_cat_id=5');
            $total_month_paid = $paid_mon_query[0]->paid + $startfee - 1;
            foreach ($inputall['fee_head'] as $selected) {
                ...
                }else if ($selected == 5) { //monthly
                    $amount = $inputall['fee_amt'][$selected];
                    $trans_fee_amt =0;
//                    $pm_month_no = $startfee + 1;
                    ...
                    for ($m = 1; $m <= $no_of_month; $m++) {
                        ...
                        $this->dbconnection->insert("fee_transaction_det", array('amount' => $amount, 'month_no' => $pm_month_no,
                            'fee_cat_id' => $selected, 'month_desc' => $month_arr[$pm_month_no].'-QTR-'. $inputall['noofqtr'], 'fee_trans_head_id' => $fee_transac_id,
...
//        }
    ...
    public function download_receipt() {
        ...
        $query_transaction = $this->db->query("select f1.*,f2.class_fee_head_id,f2.stud_category,group_concat(distinct(f2.fee_cat_id)) as fee,count(case when f2.month_no<>0 and f2.fee_cat_id=5 then f2.month_no end) as m,min(case when f2.month_no<>0 and f2.fee_cat_id=5 then f2.month_no end) as from_month,max(f2.due_month_no) as d from fee_transaction_head f1, fee_transaction_det f2 where f1.id=$fee_transaction_id and f1.response_message=0 and f1.status=1 and f1.id=f2.fee_trans_head_id group by f1.id");
        ...
        $student = $this->dbconnection->select('student', 'id,roll,course_id,transport_amt,'
                ...
                . ' email_address, phone,start_fee_month, dob, status, date_created, created_by,last_date_modified, last_modified_by', 'id = ' . $this->uri->segment(6));
