...
class Offline_payment_previous extends CI_Controller {
...
//        $this->db->db_debug=TRUE;
        ...
        $this->academic_session = $this->dbconnection->select("accedemic_session", "max(id) as fin_year,start_date,end_date,session", "status='Y' and active='N'");
        ...
        $this->page_name = 'offline_payment_upload_pre';
        $this->customview = '';
    }
...
//         }
    ...
    public function get_admsn_no() {
        ...
        $stud_qry = $this->dbconnection->select_order('student', 'id,admission_no,CONCAT(first_name," ",middle_name," ",last_name) as name', 'class_id=' . $this->input->post('class') . ' and section_id=' . $this->input->post('section') . ' and status = "N"  ', 'name', 'ASC');
        $return = '';
...
//            $max_year = $this->dbconnection->select('accedemic_session', 'max(id) as max_year','active="Y" and status="Y"');
        ...
        $fee_session_query = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$class and to_class_id>=$class) and course=" . $course_id . " and status='Y' and year=2018");
...
//                        }
            ...
            $onetime_fee_paid = $this->db->query('SELECT * from fee_transaction_det where fee_trans_head_id in( select id from fee_transaction_head where year=' . $year . ' and student_id=' . $fetch_stud[0]->id . ' and paid_status=1 and status=1) and fee_cat_id in (9,10)');
...
//            $fetch_other_fees_det_month = $this->dbconnection->select("class_fee_det as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc,(Select month_set from fee_master where id=fee.fee_id) as month_set", "fee.class_fee_head_id=" . $max_class_fee_id . " and fee_cat=3 and fee_amount!='0' and status=1");
        ...
        $transaction_history = $this->db->query("select f1.*,group_concat(distinct(f2.fee_cat_id)) as fee,"
                ...
                . " max(f2.due_month_no) as d from fee_transaction_head_accyr1 f1, fee_transaction_det_accyr1 f2 where f1.student_id=" . $stud_id
...
//            echo $fine_amount;
        ...
        $this->load->view('feepayment/collection/load_fee_collection_Previous', $data);
    }
