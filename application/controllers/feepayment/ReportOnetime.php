...
//            $recarr[] = $row->father_name;
            ...
            if ($row->monthly == 'Paid' || $row->monthly == '') {
             ...
             $sum_fees = $mons + $anns + $row->transport_amt;
...
//        if($this->session->userdata('school_id')==5){
        if(empty($feemaster)){
            $query_defaulter = $this->db->query("select $select,'' as annual,count(d2.month_no)"
                ...
                . " on f2.student_id=s.id and f2.paid_status=1 and f2.response_code=0 and f2.year={$this->academic_session[0]->fin_year} $str_query  group by s.id ");
            ...
            $query_defaulter = $this->db->query("select $select,if(count(f1.id)=0,'Unpaid','Paid') as annual,count(d2.month_no)"
                ...
                . " on f1.student_id=s.id and f1.paid_status=1 "
                . " left join ( fee_transaction_head as f2 inner join fee_transaction_det d2 on f2.id=d2.fee_trans_head_id and d2.fee_cat_id in (2,5))"
                . " on f2.student_id=s.id and f2.paid_status=1 and f2.response_code=0 $str_query and f1.year={$this->academic_session[0]->fin_year} and f2.year={$this->academic_session[0]->fin_year} group by s.id ");
    ...
    public function fee_projection_report() {
        ...
        foreach ($class_qry as $cls) {
            ...
            foreach ($section_qry as $sec) {
                ...
                foreach ($stud as $row) {
                    $collected_qry = array();
//                  $collected_qry=$this->dbconnection->select("fee_trans_det", "student_id,sum(amount) as monthly, sum(month_no) as mon", "response_code=0 and student_id=$row->student_id and Month(payment_date)=$month and paid_status=1 and fee_cat_id=2");
...
//                $fine_qry = $this->dbconnection->select("fee_transaction_det", "id,fee_trans_head_id,month_no,amount", "fee_trans_head_id in (select id from fee_transaction_head where student_id=$q->stud and paid_status=1 and response_code=0 and MONTH(payment_date)='$month' and year={$this->academic_session[0]->fin_year} $str_query1 $strquery3 ) and fee_cat_id=0");
    ...
    public function monthlyfees_wise_fee_summary() {
        ...
        $fee_details=array();
        $fetch_transaction_date=$this->db->query("SELECT fh.id,group_concat(distinct(fd.fee_cat_id)) fee_cat,fd.class_fee_head_id, fh.payment_method,fh.transaction_id,DATE_FORMAT(fh.payment_date, '%d-%m-%Y') payment_date,fh.total_amount,fh.response_message,fh.student_id,s.admission_no,concat(s.first_name,' ',s.middle_name,' ',s.last_name) as name,s.class_id,s.section_id,s.stud_category,s.course_id,s.roll,s.transport_amt,s.start_fee_month,GROUP_CONCAT(distinct(fd.month_no) order by fd.month_no) month_details,fh.discount_amount,fh.receipt_no,fh.remarks,fh.mode FROM `fee_transaction_head` fh inner join student s on fh.student_id=s.id inner join fee_transaction_det fd on fd.fee_trans_head_id=fh.id WHERE $str_query and fh.paid_status=1 and  fh.response_code=0 $str_query1
