...
//           echo (strtoupper($this->session->userdata('user_id')));die();
...
//                			$student_registered_query = $this->dbconnection->select("student", "*", "status='Y' and registered_status=1");
		...
		elseif ((strtoupper($login_tp) == 'BOOKSTORE') OR (strtoupper($login_tp) == 'CLOTHSTORE') OR (strtoupper($login_tp) == 'GENERALSTORE') OR (strtoupper($login_tp) == 'GROCERYSTORE')) {
		...
		// elseif (($login_tp == 'student') AND (strtoupper($this->session->userdata('user_id')) == '317')) {
    ...
    public function admin_dashboard() {
       ...
       public function multistudents_dashboard() {
		...
		$strsql = "SELECT sab.admission_no, concat(std.first_name,' ',std.middle_name, ' ', std.last_name)  AS fullname,  "
						...
						. "HAVING sab.status = 'Y' AND sab.admission_no = '" . $usr_name[1] . "'"; 

//AND sab.approved = 'Y' 
		$query = $this->db->query($strsql);
		$this->data['student_pay'] = $query->result();

/*		$strsql = "SELECT admission_no, SUM(amt_charged)  AS 'tot_amt_charged', approved, status "
						. "FROM student_acc_book "
						. "WHERE admission_no = '" . $usr_name[1] . "' GROUP BY admission_no "
						. "HAVING sab.status = 'Y' AND sab.approved = 'N'"; 

		$unbilled = $this->db->query($strsql);
//		$this->data['student_unbilled'] = $unbilled->result();
*/
		$this->data[$this->session->userdata('login_type') . '_message'] = 0;
