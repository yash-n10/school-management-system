...
//        $max_year = $this->dbconnection->select('accedemic_session', 'max(id) as max_year', "active='Y'");
        ...
        $other_fee = $this->db->query('SELECT *,fee_amount,'
                ...
                . 'class_fee_head_id=' . $max_class_fee_id . ' and status=1 and fee_amount!=0');
...
//        $qhalmonth=array_column($query_half_trans->result_array(), 'halfyearly_fee_id');
//        print_r(array_count_values($qhalmonth));
//        print_r($qhalfmonth);
//        die();
...
//        if($this->session->userdata('school_id')==25){$day=25;}
        $current_month = $month;
        $pais_status = array();
...
//                } else {
////                    $nn=$i-1;
////                    if($checked_status[$nn]=='')
////                        $checked_status[$i] = 'disabled';
////                    else
//                    $checked_status[$i] = '';
//                }
//            }
...
//                echo $i.'<br>';
                ...
                $dbq = $this->db->query("SELECT id,fee_name,month_set,(SELECT fee_amount FROM class_fee_det where fee_cat=4 and stud_cat=$student_cat and class_fee_head_id=$max_class_fee_id and status=1 and fee_id=fee_master.id) fee_amount FROM `fee_master` WHERE fee_cat_id=4 and FIND_IN_SET('$i', month_set) group by id")->result();
//                    print_r($dbq);
...
//        if ((($this->session->userdata('school_id') == 24) && ($total_month_paid < 3) && ($count >= 3 - $total_month_paid)) || ($this->session->userdata('school_id') == 25) && ($total_month_paid < 2) && ($count >= 2 - $total_month_paid) ) {
//            $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
//        }
...
//        $due_month=3;
//        $day=18;
//        if($this->session->userdata('school_id')==24 && $day>$this->school[0]->last_pay_date) {
//        $fine_condn=0;
        $due_month_corrected = $due_month;
//        echo $due_month.'<br>';
//        $due_month_corrected_condn='';
...
//        echo $due_month_corrected.'<br>';
        
//        if ($day > $this->school[0]->last_pay_date && $total_month_paid < ($month - 3)) {
...
//                }else{
//                    $quer_fine = $this->dbconnection->select("class_fee_det", "fee_amount", 'class_fee_head_id=' . $max_class_fee_id . ' and status=1 and stud_cat=0 and fee_cat=0 and '
//                        . '( fine_condition=4 and no_of_months > "' . $due_month . '") ');
//
//                }
...
//        $fee_trans = $this->dbconnection->select('accedemic_session', 'max(id) as max_id', 'status="Y" and active="Y" ');
...
//        $class_fee_head_id = $this->dbconnection->select('class_fee_head', 'id', ' year=' . $class_fee_head_year . '  and (from_class_id <=' . $student_class_id . ' and  to_class_id >=' . $student_class_id . ') and course=' . $student[0]->course_id . ' and status="Y"');
        ...
        if (isset($_POST['total1'])) { //monthly fee pop up button id
            ...
            if (!empty($this->input->post('half_yearly_fee'))) {
                $monthfeeamt = $monthfeeamt + array_sum($this->input->post('half_yearly_fee'));
            }

//            echo $sumhalfy=array_sum($this->input->post('half_yearly_fee'));
//                
//                die();
//            echo $final_total.'-'.$monthfeeamt.' '.$fine_amount.' '.$transport_fee_amt;
//            die();
            if ($final_total != ($monthfeeamt + $fine_amount + $transport_fee_amt)) {
        ...
        if ($final_total == 0) {
            $this->session->set_flashdata('redirectmsg', 'Sorry you cant Proceed to Payment as Amount is Zero.');
            redirect(base_url('payment'), 'refresh');
        } else {
            ...
            if ($fee_cat_id == 2) { //monthly
                $amount = ($final_total - $fine_amount - $annual_last_month_amount - $transport_fee_amt) / $month_no;
...
//---------------- Applying Fine if any -------------------------------//
        ...
        $due_month = $count;
//        if($this->session->userdata('school_id')==24 && $day>$this->school[0]->last_pay_date && $total_check_month>=($month - 3)) {
        if ($day > $this->school[0]->last_pay_date && $total_check_month >= ($month - 3)) {
//        $chk=$total_paid_month+1;
//        if ($day > $this->school[0]->last_pay_date && $chk <= ($month - 3)) { //date wise fine
//            if ($due_month == 0 && $this->session->userdata('school_id') == 25) {
//                $due_month = 0.5;
//            } elseif ($due_month != 0) {
                $due_month = $due_month + 1;
...
//        if (($this->session->userdata('school_id') == 24) && ($total_paid_month < 3) && ($total_pay_month >= 3 - $total_paid_month)) {
...
//            if(!in_array($i, $qhalfmonth)) {
            $dbq = $this->db->query("SELECT id,fee_name,month_set,(SELECT fee_amount FROM class_fee_det where fee_cat=4 and stud_cat=$student_cat and class_fee_head_id=$max_class_fee_id and status=1 and fee_id=fee_master.id) fee_amount FROM `fee_master` WHERE fee_cat_id=4 and FIND_IN_SET('$i', month_set)")->result();
...
//        $token['value']=$this->token_hash;
        echo $table_html . '|' . $total_fee . '|' . $total_paid_month . '|' . $this->school[0]->annual_month;
    }
    ...
    public function btn_download_pop_load() {
        ...
        $query_transaction = $this->db->query("select f1.*,f2.class_fee_head_id,group_concat(distinct(f2.fee_cat_id)) as fee,group_concat(distinct(f2.halfyearly_fee_id)) as halfyearly_fee_id,count(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) as m,min(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) as from_month,max(f2.due_month_no) as d from fee_transaction_head f1, fee_transaction_det f2 where f1.id=$fee_transaction_id and f1.response_message=0 and f1.status=1 and f1.id=f2.fee_trans_head_id group by f1.id");
        ...
        $month_session = $str;
//        $max_year1 = $this->dbconnection->select('accedemic_session', 'max(id) as max_year1', 'status="Y" and active="Y"');
