...
//        $half_yearly_fee_history = $this->db->query('SELECT * from fee_trans_det where student_id=' . $student_id . ' and year=' . $year . ' and fee_cat_id=4 and response_message is not null');
        $half_yearly_fee_history = $this->db->query('SELECT * from fee_transaction_det where fee_trans_head_id in( select id from fee_transaction_head where year=' . $year . ' and student_id=' . $student_id . ' and response_message is not null and status=1) and fee_cat_id=2');
...
//        }
        for ($i = $total_month_paid+1; $i <= 12; $i++) {
                ...
                if ($i <= $newmonth && $payment_gateway!='WORLDLINE') { // remove pgw condn
                    $checked_status[$i] = 'checked';
                    $count++;
                    $chkcount++;
                }elseif ($i == $total_month_paid+1 && $payment_gateway=='WORLDLINE') { //remove this line
                    $checked_status[$i] = 'checked';
                    $count++;
                    $chkcount++;
                }else {
        ...
        $dischk=($payment_gateway!='WORLDLINE')?$chkcount + 2:$chkcount + 1; // remove this line
//        for ($st = ($chkcount + 2); $st <= 12; $st++) {   //have uncomment this line
        for ($st = $dischk; $st <= 12; $st++) { //remove this line
...
//        $fine_condn=0;
        ...
        if (!empty($this->school[0]->start_pay_date) && $student[0]->fine_waiver!='YES') {//only if fine is applicable
            ...
            if ($fine_apply_status == 1) {
                ...
                if($payment_gateway=='WORLDLINE' && $due_month>1) {
                        $fine_amount=$this->get_fine_amt($max_class_fee_id,$due_month,1);
                }else{
...
//-----------Late Fine --------------- //    
        ...
        if (!empty($this->school[0]->start_pay_date) && $student[0]->fine_waiver!='YES') {  //only if fine is applicable
            ...
            if ($fine_apply_status == 1) {
                $fine_amount=$this->get_fine_amt($max_class_fee_id,$actualfinemonth,$due_month);
            }
        }
//-------------------------------- //
...
//        $token['value']=$this->token_hash;
    ...
    public function get_fine_amt($max_class_fee_id,$actualfinemonth,$due_month) {
        
        $fine_amount=0;
                if ($this->school[0]->fine_type_checkbox == 'ADJUSTABLE') {

                    $duemonthcancel = $actualfinemonth - $due_month;
                    $quer_fine_actual = $this->dbconnection->select("class_fee_det", "fee_amount", 'class_fee_head_id=' . $max_class_fee_id . ' and status=1 and stud_cat=0 and fee_cat=0 and '
                            . ' (( fine_condition=1 and no_of_months=' . $actualfinemonth . ') or ( fine_condition=2 and no_of_months<' . $actualfinemonth . ') '
                            . ' or ( fine_condition=3 and no_of_months <= ' . $actualfinemonth . ') '
                            . ' or ( fine_condition=4 and no_of_months > ' . $actualfinemonth . ') '
                            . ' or ( fine_condition=5 and no_of_months >= ' . $actualfinemonth . '))');
