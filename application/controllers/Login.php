...
//print_r($_POST);
	...
	public function submitLogin($user = '', $pass = '', $schoolid = '') {
		...
		if (preg_match('/-/', $this->input->post('username'))) { 	//** We need a - in USERNAME to SEPARATE School Code from User Name
			$school_code_arr = explode('-', $this->input->post('username'));
            
			if ($school_code_arr[0] == '@') { 				// ** for Application ADMIN USER
				$school_id = 0;
				$school_name = '';
			} else {
			...
			if ($school_id !== FALSE) { 		//** When we have a school ID
				...
				if (count($user) == 1) {
					...
					if ($user[0]->bad_pw_tally > 5 && $user[0]->last_bad_pw_ts > (time() - 300)) { 	//** Locked out!
						...
						} else { 					//** If BAD PASSWORD, then we need to do something about that!
					...
					} else { 					//** In case, NO MODERN Password Hash present - go for legacy upgrade
				...
				if ($valid_user) {
					$user_grp_name = array();
					$db='crmfeesclub_'.$school_id;
					$user_grp=$this->db->query("SELECT id,group_type FROM '$db'");
					$user_grp = $this->dbconnection->select("user_group", "id,group_type");
					foreach ($user_grp as $r) {
						$user_grp_name[$r->id] = $r->group_type;
					}
					$this->user_grp_name = $user_grp_name;
					// print_r($this->user_grp_name);die();
					$login_type = $this->user_grp_name[$user[0]->user_group_id];
				
//					1: Administrator	2: Supervisor		3: School		4: Student		5: Parent		6: Teacher
//					7: Principal		8: Office			9: Librarian		10: Accountant	,etc.
