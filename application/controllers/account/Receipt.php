...
//            redirect(base_url(), 'refresh');
        ...
        $voucher = $this->db->query("SELECT count(bill_gen_no) as vno FROM voucher where voucher_type='Receipt' and module_name in('Receipt','Sale','Purchase_Return') group by bill_gen_no;")->result();
        $v_no = count($voucher) + 1;
        $date = date('ymd');
        $this->data['voucher_no'] = 'RCP/' . $date . '/' . $v_no;
