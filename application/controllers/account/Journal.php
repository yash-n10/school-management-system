...
//            redirect(base_url(), 'refresh');
        ...
        $voucher = $this->db->query("SELECT count(bill_gen_no) as vno FROM voucher where voucher_type='Journal' and module_name ='Journal' group by bill_gen_no;")->result();


        $v_no = count($voucher) + 1;
        $date = date('ymd');
// $this->data['voucher_no'] = 'PAY/'.$date.'/';
        $this->data['voucher_no'] = 'JUR/' . $date . '/' . $v_no;
