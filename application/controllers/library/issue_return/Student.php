...
class Student extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->data['page_name'] = 'issue_return/student';
        $this->data['page_title'] = 'Issue Return (STUDENT)';
    ...
    function get_detail()
    {
        ...
        $data = $this->db->query("SELECT t1.*,t2.class_name,t3.sec_name  FROM student as t1 JOIN class as t2 ON t1.class_id=t2.id JOIN section as t3 ON t3.id=t1.section_id WHERE t1.admission_no='$no'")->result();
        $stu_id         = $data[0]->id;
        ...
        $update_tmp_book_issue = $this->db->query("SELECT t1.*,t2.acc_no,t2.isbn_no,t3.name,t3.book_publisher_id,t3.almirah_no,t3.rack_no,t4.fine_stud as fine FROM book_issue as t1 JOIN book_detail as t2 ON t2.id=t1.book_detail_id JOIN book as t3 ON t3.id=t1.book_id JOIN book_category as t4 ON t4.id=t3.book_category_id WHERE t1.created_by=$this->user_id AND t1.st_id=$stu_id AND t1.issue_user_type='STU' AND t1.status='Y' AND t1.issue_status='L' ORDER BY t1.id DESC")->result();
        ...
        $data_tmp_book_issue = $this->db->query("SELECT t1.*,t2.acc_no,t2.isbn_no,t3.name,t3.book_publisher_id,t3.almirah_no,t3.rack_no,t4.fine_stud as fine FROM book_issue as t1 JOIN book_detail as t2 ON t2.id=t1.book_detail_id JOIN book as t3 ON t3.id=t1.book_id JOIN book_category as t4 ON t4.id=t3.book_category_id WHERE t1.created_by=$this->user_id AND t1.st_id=$stu_id AND t1.issue_user_type='STU' AND t1.status='Y' ORDER BY t1.id DESC")->result();
