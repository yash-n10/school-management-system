...
//            $this->db->db_select('crmfeesclub');
        ...
        switch ($sckool_gw[0]->payment_gateway) {
            case 'WORLDLINE':
                ...
                break;
            case 'CCAVENUE':
                if($this->input->post('autostatus')=='true') {
...
//                ini_set('display_errors', 1);
    ...
    public function receipt()
    {
        ...
        $data['page_title'] = 'receipt cancel';
        $data['section'] = 'backend';
        $query=$this->db->query("SELECT id,description FROM crmfeesclub.school where status=1");
        $query=$query->result();
        // echo "<pre>";print_r($query);die();
        $data['school_list']=$query;
        $this->load->view('index',$data);
    }

    public function receipt_cancel()
    {
        // print_r($this->input->post());
        $school_id=$this->input->post('school_id');
        $receipt_no=$this->input->post('receipt_no');
        // echo "<br>";
        print_r("UPDATE crmfeesclub_$school_id.fee_transaction_head set paid_status=0 and status=0 WHERE receipt_no='$receipt_no'");
        $query=$this->db->query("UPDATE crmfeesclub_$school_id.fee_transaction_head set paid_status=0 and status=0 WHERE receipt_no='$receipt_no'");
        if($query){
            echo "Success";
        }
        else{
    ...
    {
        $data['page_name'] = 'chargeback';
        $data['page_title'] = 'chargeback cancel';
        $data['section'] = 'backend';
        $query=$this->db->query("SELECT id,description FROM crmfeesclub.school where status=1");
        $query=$query->result();
        // echo "<pre>";print_r($query);die();
    ...
    public function chargeback_cancel()
    {
        // print_r($this->input->post());
        $school_id=$this->input->post('school_id');
        ...
        // echo "<br>";
        print_r("UPDATE crmfeesclub_$school_id.fee_transaction_head set paid_status=0 and status=0 and remarks='$remarks' WHERE transaction_id='$transaction_id'");
        $query=$this->db->query("UPDATE crmfeesclub_$school_id.fee_transaction_head set paid_status=0 and status=0 WHERE receipt_no='$receipt_no'");
        if($query){
            echo "Success";
        }
