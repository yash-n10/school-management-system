...
class Payment extends CI_Controller {
	...
	public function index() {
    	...
    	// $outstanding=$this->db->query("SELECT * FROM student_acc_book where admission_no='$admission_no' ");
    	// $outstanding=$outstanding->result();
    	// // echo "<pre>";print_r($outstanding);
    	$strsql = "SELECT sab.admission_no, concat(std.first_name,' ',std.middle_name, ' ', std.last_name)  AS fullname,  "
						...
						. "HAVING sab.admission_no = '" . $admission_no . "'"; 

		$outstanding = $this->db->query("SELECT sab.admission_no, concat(std.first_name,' ',std.middle_name, ' ', std.last_name)  AS fullname,  "
						. "SUM(sab.amt_charged)  AS 'tot_amt_charged', SUM(sab.amt_paid) AS 'tot_amt_paid' "
						. "FROM student_acc_book sab, student std "
						. "WHERE sab.admission_no = std.admission_no GROUP BY sab.admission_no "
						. "HAVING sab.admission_no = '$admission_no'");
    	...
    	// 	echo "<br>";	print_r($key); echo "<br>";
    	// 	$count += $key->amt_charged-$key->amt_paid;
    	// }
    	$outstanding=$outstanding->result();
    	$outstanding=$outstanding[0];
    	// print_r($outstanding);die();
...
public function request()
{
		...
		$max_class_fee_id 	 = $year_class[0]->max_id;
            ...
            $this->dbconnection->insert("store_transaction", array('student_id' => $student_id, 'request_status' => 1, 'year' => $max_sesion_id,'status'=>1,'total_amount' => $amount, 'paid_by' => $this->session->userdata('user_id'), 'payment_date' => date('Y-m-d H:i:s'), 'date_created' => date('Y-m-d H:i:s'),
...
// print_r($insert_to_db);die();
		if ($insert_to_db == 'YES') {
			...
			// print_r('crmfeesclub_' . $school_id);die();
			// print_r($trans_id);echo "<br>";echo '<pre>';print_r($data); die();
