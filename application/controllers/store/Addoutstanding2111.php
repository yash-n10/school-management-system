...
class Addoutstanding extends CI_Controller {
	...
	public function __construct() {
		...
		$this->page_title = 'Initail Outstandings';
		$this->section = 'store';
	...
	public function index() {
		...
		$this->data['navi'] 		= 'outstandinglist_view';    //'store';
		$this->data['right_access']	= $this->right_access;
             ...
             $this->data['arr_acc_book'] = $this->dbconnection->select_order('student_acc_book','*,'
					...
					. '1', 'remarks="Initial Outstandings"', 'adm_no', 'ASC');
		...
		if (count($this->input->post('pro')) == 0) {
			...
			for ($i = 0; $i < count($this->input->post('pro')); $i++) {
				$field = array(
						...
						'bill_date' 		=> $this->input->post('date_received')[$i],
