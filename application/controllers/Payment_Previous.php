...
class Payment_Previous extends CI_Controller {
    ...
    public function __construct() {
        ...
        if ($this->id != 0) {
         ...
         $this->academic_session = $this->dbconnection->select("accedemic_session", "max(id) as fin_year", "status='Y' and active='N'","id","DESC");
         // print_r($this->academic_session);die();
    ...
    public function index() {
        ...
        $this->academic_log_query=$this->dbconnection->select('student_class_acedemic_log','*','student_id='.$student_id,"id","DESC");
        ...
        $fetch_instant_fees_det = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $student_id . " and year=$this->previousSessionID and paid_status!=1 and status='Y'");
            ...
            if(empty($total_fee)) {
                ...
                $query_defaulter_annual = $this->db->query("select f1.id,count(f1.id) as annual from fee_transaction_head f1,fee_transaction_det f2 where f1.id=f2.fee_trans_head_id and f1.status=1 and f1.paid_status=1 and f2.fee_cat_id=1 and f1.year=$this->previousSessionID and f1.student_id=".$student_id)->result();
                 ...
                 $dmonth='12'-$paid_month;
                 $dann=$query_defaulter_annual[0]->annual;
            }
         // $max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $course_id . " and status='Y' and year=$this->previousSessionyear");
         $max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $course_id . " and status='Y' and year=$this->previousSessionID");
         $year=$this->previousSessionID;
         // print_r($max_year);die();
        $fee_session_year = $max_year[0]->accd_session;
        ...
        $newmonth=(12-$total_month_paid);
        // $newmonth=(12-$total_month_paid) + $newmonthh;
        // die();
        ...
        $fine_amount=0;
         $due_month = 3 ;
        // echo $due_month = $count ;
        // echo $due_month = $newmonth);
    // die();
        ...
        $data = array(
            ...
            'page' => 'student',
            'psession' => $this->psession,
            'fee_type1' => $school[0]->fee_type1,
            'fee_type2' => $school[0]->fee_type2,
            'student' => $student,
            'student_type' => $student_type,
            'school' => $school,
            'academic_session' => $this->p_academic_session,
            'msg1' => '',
            'msg2' => '',
...
//        $month=05;
        ...
        $student = $this->dbconnection->select('student', 'id,reference_no,first_name, middle_name, last_name, stud_category,admission_no,'
         ...
         $this->academic_log_query=$this->dbconnection->select('student_class_acedemic_log','*','student_id='.$user[0]->student_id,"id","DESC");
        $student_class_id= $this->academic_log_query[0]->class_id; //old class
        $course_id= $this->academic_log_query[0]->course_id; //old course
        $student_type = $student[0]->student_type;
        $student_cat = $student[0]->stud_category;
        ...
        $year_class = $this->dbconnection->select('class_fee_head', 'max(year) as year, max(id) as max_id', "(from_class_id <=$student_class_id and  to_class_id >=$student_class_id) and course=" . $course_id . " and status='Y' and year=$this->previousSessionID");
        // print_r($year_class);
        // die();
...
//        echo $init_mont.' '.$fin_mont;
        ...
        if (isset($_POST['total1'])) { //monthly fee pop up button id
            ...
            // echo $monthfeeamt;
            // echo '<br>';
            // echo $instantfee_amount;
            // echo '<br>';
            // echo $transport_fee_amt;
            // die();

             if((($this->session->userdata('school_id')==25) && ($student_id==581)) ||(($this->session->userdata('school_id')==25) && ($student_id==2756))||(($this->session->userdata('school_id')==25) && ($student_id==1404))||(($this->session->userdata('school_id')==25) && ($student_id==1108))||(($this->session->userdata('school_id')==25) && ($student_id==670)) ||(($this->session->userdata('school_id')==25) && ($student_id==411))||(($this->session->userdata('school_id')==25) && ($student_id==416))||(($this->session->userdata('school_id')==25) && ($student_id==429))||(($this->session->userdata('school_id')==25) && ($student_id==461))||(($this->session->userdata('school_id')==25) && ($student_id==498))||(($this->session->userdata('school_id')==25) && ($student_id==600))||(($this->session->userdata('school_id')==25) && ($student_id==603))||(($this->session->userdata('school_id')==25) && ($student_id==639))||(($this->session->userdata('school_id')==25) && ($student_id==650))||(($this->session->userdata('school_id')==25) && ($student_id==653))||(($this->session->userdata('school_id')==25) && ($student_id==658))||(($this->session->userdata('school_id')==25) && ($student_id==670))||(($this->session->userdata('school_id')==25) && ($student_id==664))||(($this->session->userdata('school_id')==25) && ($student_id==672))||(($this->session->userdata('school_id')==25) && ($student_id==683))||(($this->session->userdata('school_id')==25) && ($student_id==803)))
             {
                if ($final_total != ($fine_amount+$readmsnfine + $transport_fee_amt + $half_yearly_amount + $oth_amount + $instantfee_amount)) {
...
//       $this->db->db_debug=TRUE;
        ...
        $student_class_id= $this->academic_log_query[0]->class_id; //old class
        $course_id= $this->academic_log_query[0]->course_id; //old course
        // $student_class_id = ($student[0]->class_id)-1;
        // $student_class_id = $student[0]->class_id;
        $max_year = $this->dbconnection->select('class_fee_head', 'max(year) as max_year, max(id) as max_id', "(from_class_id <=$student_class_id and  to_class_id >=$student_class_id) and course=" . $course_id . " and status='Y' and year=$this->previousSessionID");
...
//---------------- Applying Fine if any -------------------------------//
        ...
        // $due_month = $count;
// $daty=5;
        // if ($day > 1 && $total_check_month >= ($count - 3) && $actualfinemonth!=0) {
        ...
        if ($fine_rule[0]->cnt != 0 ) {
            if ($fine_rule[0]->mdue_month < $due_month) {
                $fine_rule_fetch = $this->dbconnection->select("fine_rule", "max_day,remain", "due_month={$fine_rule[0]->mdue_month} and from_day>=1 and to_day<=31");
            }else {
                $fine_rule_fetch = $this->dbconnection->select("fine_rule", "max_day,remain", "due_month=$due_month and from_day<=1 and to_day>=31");
...
//-----------Late Fine --------------- //   
    ...
    //                    }
        ...
        // if ($fine_apply_status == 1 && $fine_amount!=0) {
            ...
            $table_html .= '<tr>';
            $table_html .= '<td>Fine (for 3 Month)</td>';

            $table_html .= '<input type="hidden" name="fine_due_month" value="3">';
            $table_html .= '<td><input type="text" readonly="true" style="border: 0px;width:65px;background:inherit" name="fine_amt" value="250"></td>';
