...
class Payment extends CI_Controller {
	...
	public function __construct() {
		...
		$acaedmic_session_id=$this->academic_session[0]->fin_year;
		// print_r($acaedmic_session_id);die();
		// $fetch_startyr = isset($this->academic_session[0]->start_date) ? explode('-', $this->academic_session[0]->start_date) : array('0');
		// // print_r($fetch_startyr);die();
		// $this->session_start_yr = reset($fetch_startyr);
		$this->session_start_yr=$acaedmic_session_id;
		$this->academic_session_pre = $this->dbconnection->select("accedemic_session", "max(id) as fin_year", "status='Y' and active='N'","id","DESC");
		// print_r($this->academic_session_pre);die();
        ...
        //COMMENTED BY YASH FOR COLLECTION OF 2020-21 PAYMENT IN 2021-22
		
		// if((($this->session->userdata('school_id')==25) && ($student_academicyear_id==2)) || (($this->session->userdata('school_id')==29) && ($student_academicyear_id==2)))
		// { ?>
		 	<script>
		// 		alert(" Sorry! Your Payment For 2019-2020 Session Has Been Clossed. Please Contact your School Administrator !");
		// 		window.location.assign("http://erp.feesclub.com");
  //           </script>
		 <?php 
		// }
		...
		$this->academic_log_query=$this->dbconnection->select('student_class_acedemic_log','*','student_id='.$student_id);
		if(!empty($this->academic_log_query)) {
			$student_class_id= $this->academic_log_query[0]->class_id; //old class
			$course_id= $this->academic_log_query[0]->course_id; //old course
			...
			$check_fee_struc_zero=$this->db->query("SELECT sum(fee_amount) famt FROM class_fee_det cd inner join class_fee_head ch on cd.class_fee_head_id=ch.id "
						. "where (ch.from_class_id<=$student_class_id and ch.to_class_id>=$student_class_id) and ch.course=$course_id and ch.status='Y' and ch.year<=$this->previousSessionyear and cd.stud_cat=$student_cat "
		...
		if(empty($total_fee)) {
			...
			$query_defaulter = $this->db->query("select count(f2.month_no) as monthly, f1.id from fee_transaction_head f1,fee_transaction_det f2 where f1.id=f2.fee_trans_head_id and f1.status=1 and f1.paid_status=1 and f2.fee_cat_id=2 and f1.year=$this->previousSessionID and f1.student_id=".$student_id)->result();

			$query_defaulter_annual = $this->db->query("select f1.id,count(f1.id) as annual from fee_transaction_head f1,fee_transaction_det f2 where f1.id=f2.fee_trans_head_id and f1.status=1 and f1.paid_status=1 and f2.fee_cat_id=1 and f1.year=$this->previousSessionID and f1.student_id=".$student_id)->result();

			$this->instant_fee_prev = $this->dbconnection->select("student_other_fee as fee", "fee.*, (Select fee_name from fee_master where id=fee.fee_id) as fee_desc", "fee.student_id=" . $student_id . " and year=$this->previousSessionID and paid_status!=1 and status='Y'");
                // print_r($fetch_instant_fees_det);
                // die();
			$paid_month=$query_defaulter[0]->monthly;
			$this->dmonth='12'-$paid_month;
			$dann=$query_defaulter_annual[0]->annual;
		}
...
//        $this->token_hash = $this->security->get_csrf_hash();
 ...
 //COMMENTED BY YASH FOR COLLECTION OF 2020-21 PAYMENT IN 2021-22
		...
		// if(($this->session->userdata('school_id')==8) ||($this->session->userdata('school_id')==25) ||($this->session->userdata('school_id')==29)||($this->session->userdata('school_id')==33)) {
		// 	// if(($this->session->userdata('school_id')==8) || ($this->session->userdata('school_id')==5) ||($this->session->userdata('school_id')==25) ||($this->session->userdata('school_id')==29)) {
		// 	// echo "<pre>";print_r($this->academic_log_query);die();
		// 	if(!empty($this->academic_log_query))
		// 	{
		// 		// echo "string";die();
		// 		// echo $this->dmonth;die();
		// 		// 0<12 
		// 		if(($this->dmonth!=0 ) || ((!empty($this->instant_fee_prev))))
		// 		{?>
		 			<script>
		// 				alert("Please Clear the dues of Session 2019-2020");
		// 				window.location.assign("http://erp.feesclub.com/Payment_Previous");
		// 			</script>
				<?php
  //           //     redirect(base_url('Payment_Previous'));
  //           // die();
		// 		}
		// 	}
  //             }
 ...
 //       print_r("Token Name " . $this->security->get_csrf_token_name());
  //      print_r("Token_hash " . $this->security->get_csrf_hash());
   //     die();

		// if(($this->session->userdata('school_id')==5) && ($student_id==2628)){       }
		...
		//YEAR WILL BE STUDENT'S ACADEMIC YEAR NOT BY ACTIVE SESSION DONE BY YASH
		
		//TO SHOW FEES PAID IN PREVIOUS MONTH
		// $year = $this->academic_session[0]->fin_year;
		$year=$student[0]->student_academicyear_id;

		$active_ses_id= $this->dbconnection->select('accedemic_session','*',"id='$stud_acedemic_session_id'");
		$active_=$active_ses_id[0]->active;
      
		if($active_=='U') {
			$max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year=".$stud_acedemic_session_id);
		} else {
			if($this->session->userdata('school_id')==35) {
				$max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year=$this->session_start_yr");
			} 
			// else if(($this->session->userdata('school_id')==5) && ($student_academicyear_id!=$year)) {
			// 	$max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year<=$this->session_start_yr");
			// } 
			else if(($this->session->userdata('school_id')==5) ||($this->session->userdata('school_id')==25) ||($this->session->userdata('school_id')==29)||($this->session->userdata('school_id')==33)) {
				// print_r($stud_acedemic_session_id);die();
				$max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year=".$stud_acedemic_session_id);
			} else {
				$max_year = $this->dbconnection->select("class_fee_head", "max(year) as accd_session, max(id) as max_id", "(from_class_id<=$student_class_id and to_class_id>=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year<=$this->session_start_yr");
			}
...
// TO DISABLE FEES EXCEPT TUTION FEE AND COMP FEE IN DAV BARIATU
//      if($this->session->userdata('school_id')==29){
//         $monthly_fee = $this->db->query('SELECT fee_id,fee_amount,'
//                 . '(select fee_name from fee_master where id=class_fee_det.fee_id) as fee_name'
//                 . ' FROM class_fee_det where fee_cat=2 and fee_id in (1,5) and stud_cat=' . $student_cat . ' and '
//                 . 'class_fee_head_id=' . $max_class_fee_id . ' and status=1');
// }
        ...
        $other_fee = $this->db->query('SELECT cd.*,cd.fee_amount,fm.fee_name FROM class_fee_det cd inner join fee_master fm on fm.id=cd.fee_id'
                . ' where cd.fee_cat=3 and cd.class_fee_head_id='. $max_class_fee_id . ' and cd.status=1 ');


        // echo "<pre>";print_r($max_class_fee_id);die();
...
//                . 'class_fee_head_id=' . $max_class_fee_id . ' and status=1');
        ...
        $other_fee_paid = $this->dbconnection->select_join('fee_transaction_head a', 'a.paid_status,a.remarks,a.receipt_no,b.other_fee_id', "a.year=$year and a.student_id=" . $student_id . " and a.paid_status=1 and a.status=1 and b.fee_cat_id=3", "fee_transaction_det b", " a.id=b.fee_trans_head_id", "inner");
        // echo "<pre>";
        // print_r($other_fee_paid);
        // die();
             if (count($other_fee_paid) > 0) {
                $paid_status_other = count($other_fee_paid);
                $remark_other = $other_fee_paid[0]->remarks;
                $receipt_no_other = $other_fee_paid[0]->receipt_no;
            } else {
                $paid_status_other = 0;
                $remark_other = '';
                $receipt_no_other = '';
...
//        }
        ...
        // print_r($paid_mon_query);die();
        $total_month_paid = $paid_mon_query[0]->paid;

        list($year, $month, $day) = explode('-', date('Y-m-d'));

//CODE BY YASH TO SET NO OF MONTH =12 IF STUDENT IS IN PREVIOUS SESSION
	if($this->academic_session[0]->fin_year>$student[0]->student_academicyear_id){
		$month=3;
	}        
        $current_month = $month;
        $pais_status = array();
        ...
        for ($pm = 1; $pm <= $total_month_paid; $pm++) {
            ...
            $checkedblock_status[$pm] = 'onclick="return false;"';
            $chkcount++;
        }
        ...
        for ($i = $total_month_paid+1; $i <= 12; $i++) {
        	// CODE BY YASH 13 02 2022 TO MAKE FEE PAYMENT OF MARCH MANDATORY IN VIVEK VIDYALAYA IN FEBRUARY
        	$school = $this->school;

        	// echo $school[0]->id;die();
        	if ($school[0]->id==5) {
        		 $newmonth= $newmonth+1;
        	}
            ...
            if ($i <= $newmonth) {
                    ...
                    if($this->academic_session[0]->fin_year>$student[0]->student_academicyear_id){
                    	$checked_status[$i] = 'checked  disabled';
                    }
                    $checkedblock_status[$i] = 'onclick="return false;"'; 
                    $count++;
                    $chkcount++;
...
//                    print_r($dbq);
                    foreach ($dbq as $value) {
                        ...
                        if (empty($qhalfmonth[$value->id])) {
                            ...
                            $half_yearly_fee_id[$value->id] = $value->fee_name;
        ...
        switch($this->session->userdata('school_id')){
            case 29:
                $trans= $student[0]->transport_amt;
                if(($total_month_paid < 3) && ($count >= 3 - $total_month_paid)){ //June
                    $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
                }

                if((($student_class_id==13) || ($student_class_id==15)) && (($total_month_paid < 12) && ($count >= 12 - $total_month_paid))) {          //Class 10 & 12 March
                    $transport_fee_amt = $trans * ($count - 1);
                    // $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
                }
                ...
                if((($student_class_id==10) || ($student_class_id==12)) && (($total_month_paid < 12) && ($count >= 12 - $total_month_paid))) {          //March
                    $transport_fee_amt = $trans * ($count - 1);
                    // $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
                }
            ...
            case 25:
               $trans= $student[0]->transport_amt;
                if(($total_month_paid < 1) && ($count >= 1 - $total_month_paid)){ //May
                    $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
                }
                if(($total_month_paid < 2) && ($count >= 2 - $total_month_paid)){ //May
                    $transport_fee_amt = $student[0]->transport_amt * ($count - 2);
                }
                break;

                case 38:
                 if(($total_month_paid < 1) && ($count >= 1 - $total_month_paid)){ //April
                   $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
                }
                // if(($total_month_paid < 2) && ($count >= 2 - $total_month_paid)){ //May
                //    $transport_fee_amt = $student[0]->transport_amt * ($count - 1);
        ...
        /* -----------  --------------------- */
if($this->session->userdata('school_id')==29){
$olympiad= $this->db->query('SELECT cd.*,cd.fee_amount,fm.fee_name FROM class_fee_det cd inner join fee_master fm on fm.id=cd.fee_id'
                . ' where cd.fee_cat=3 and cd.class_fee_head_id='. $max_class_fee_id . ' and cd.status=1 and (fm.month_set= "" OR fm.month_set IS NULL) and fee_id in (138,139,140,141)');
}
        $school = $this->school;
        $data = array(
            ...
            'checkedblock_status' => $checkedblock_status, 
            'count' => $count,
...
/*	$student = $this->dbconnection->select('student', 'id,reference_no,first_name, middle_name, last_name, stud_category,admission_no,'
		...
		$student = $this->dbconnection->select('student', 'id,reference_no,first_name, middle_name, last_name, stud_category,admission_no,'
					...
					. ' class_id,(select cl.class_name from class cl where cl.id=class_id) as class_name,'
					. ' section_id,(select s.sec_name from section s where s.id=section_id) as sec_name,'
					. ' course_id,email_address, phone,status, date_created, created_by, last_date_modified, last_modified_by,start_fee_month,student_type,student_academicyear_id', 'id = ' . $student_id);
		...
		$session_start_yr=$student[0]->student_academicyear_id;
		// $max_sesion_id = $this->academic_session[0]->fin_year;
		$max_sesion_id =$student[0]->student_academicyear_id;
		$year_class = $this->dbconnection->select('class_fee_head', 'max(year) as year, max(id) as max_id', "(from_class_id <=$student_class_id and  to_class_id >=$student_class_id) and course=" . $student[0]->course_id . " and status='Y' and year<=$session_start_yr");
		...
		$fee_details = $this->db->query("SELECT sum(if(fee_cat=1,fee_amount,0)) annual, sum(if(fee_cat=2,fee_amount,0)) monthly, sum(if(fee_cat=9 or fee_cat=10,fee_amount,0)) onetime FROM class_fee_det WHERE class_fee_head_id=$class_fee_head_id and stud_cat=$student_cat and status=1");
		$fee_details = $fee_details->result();
       
//	   print_r($user[0]->student_id . " AND " . $student_id);die();
        
		$months = $this->input->post('month');
		$i = 1;
		$init_mont = 0;
		$fin_mont = 0;
		// print_r($max_sesion_id);echo "<br>";
			// print_r( $monthly_fee_paid);die();
		if (count($months) > 0) {
		...
		//CODE BY YASH TO COLLECT PREVIOUS YEAR PAYMENTS
		if($max_sesion_id!=$this->academic_session[0]->fin_year){
			// echo "string";die();
			$fin_mont=12;
			$monthly_fee_paid_no=$this->db->query('SELECT max(month_no) as month_no from fee_transaction_det where fee_trans_head_id in( select id from fee_transaction_head where year=' . $max_sesion_id . ' and student_id=' . $student_id . ' and paid_status=1 and status=1) and fee_cat_id=2');
			$monthly_fee_paid_no=$monthly_fee_paid_no->result();
			$init_mont=$monthly_fee_paid_no[0]->month_no +1 ;
		}
		// print_r($init_mont);die();
       // echo $init_mont.' -----------'.$fin_mont;die();
        ...
        if (isset($_POST['total1'])) { //monthly fee pop up button id
            ...
            if($session_start_yr!=$this->session_start_yr){
			
			$student_id=$student[0]->id;
			// print_r($student_id);
            	$query_month=$this->db->query("SELECT COUNT(month_no) as month_no FROM fee_transaction_det WHERE fee_trans_head_id in (SELECT id FROM fee_transaction_head where student_id=$student_id and paid_status=1 and year=$session_start_yr) and fee_cat_id=2");
            	$query_month=$query_month->result();
            	$query_month=$query_month[0]->month_no;
            	$month_no=12 - $query_month;
            	// print_r($month_no);die();
            	// print_r("SELECT COUNT(month_no) FROM fee_transaction_det WHERE fee_trans_head_id in (SELECT id FROM fee_transaction_head where student_id=$student_id and paid_status=1 and year=$session_start_yr) and fee_cat_id=2");die();
            }
           ...
           // print_r($final_total);echo"<br>"; print_r($monthfeeamt);die();
...
//                $this->save_transaction_details($class_fee_head_id[0]->id, $student_cat, $final_total, 0, '', 0, NULL, $fee_cat_id, $fee_transac_id, $user_id);
				...
				if($this->session->userdata('school_id')!=29) {
					...
					foreach ($otherfee as $key => $value) {
						foreach($this->input->post('other') as $keychk => $valuechk) {
							if($key==$valuechk) {
								$this->save_transaction_details($class_fee_head_id, $student_cat, $value, 0, '', 0, $key, $fee_cat_id, $fee_transac_id, $user_id);
								$cnt_other++;
									// $fetch_stock_list=$this->dbconnection->select('book_stock_master','*','fee_id='.$key);
									// $op_qty=$fetch_stock_list[0]->opening_qty;
									// $rest_qty=$fetch_stock_list[0]->rest_qty;
									// $sale_qty=$fetch_stock_list[0]->sale_qty;
									// $new_rest_qty=$rest_qty -1;
									// $new_sale_qty=$sale_qty +1;
									// $data=array(
									//   'rest_qty' => $new_rest_qty,
									//   'sale_qty' => $new_sale_qty,
									//    );
									// $this->dbconnection->update('book_stock_master',$data,'fee_id='.$key);
							}
						}
                   }
                }
...
//                   }
            ...
            $description = "FeesClub Total $final_total - AdmissionNo $admission_no - Student ID $student_id "
...
//                    }
	...
	public function save_transaction_details($class_fee_head_id, $student_cat, $amount, $month_no, $month_desc, $due_month_no, $other_fee_id, $fee_cat_id, $fee_transac_id, $user_id, $hal_fee_id = 0) {
		if ($fee_cat_id==10 or $fee_cat_id==11 or $fee_cat_id==138 or $fee_cat_id==139 or $fee_cat_id==140 or $fee_cat_id==141) {
...
//                    $school_id = $response['merchant_param1'];
		...
		if ($insert_to_db == 'YES') {
			...
			if($this->session->userdata('school_id')==29)
			{
				$trans_other_fee_query = $this->db->query("select fth.id,ftd.fee_trans_head_id,ftd.other_fee_id,ftd.amount from fee_transaction_head fth,fee_transaction_det ftd where fth.id=ftd.fee_trans_head_id and fth.response_code=0 and status=1 and paid_status=1 and ftd.fee_cat_id=3 and fth.id='$trans_id'")->result();
                
				foreach ($trans_other_fee_query as $key => $value) {
					$fee_id		= $value->other_fee_id;
					$fetch_stock_list = $this->dbconnection->select('book_stock_master','*','fee_id='.$fee_id);
					$op_qty		= $fetch_stock_list[0]->opening_qty;
					$rest_qty		= $fetch_stock_list[0]->rest_qty;
					$sale_qty		= $fetch_stock_list[0]->sale_qty;
					$new_rest_qty	= $rest_qty -1;
					...
					$this->dbconnection->update('book_stock_master',$data,'fee_id='.$fee_id);
				}
			}
...
//Audit Trail
	...
	public function  get_student_fee() {
		...
		$student_class_id = $student[0]->class_id;
		 ...
		 if(($this->session->userdata('school_id')==5) ||($this->session->userdata('school_id')==25) ||($this->session->userdata('school_id')==29)||($this->session->userdata('school_id')==33))
		{
...
//        }
		...
		switch($this->session->userdata('school_id')){
			...
			case 29:
			// CODE ADDED BY YASH TO REMOVE TRANSPORT FEE FROM DAV BARIATU 
				$transport_fee_amt=0;
			// CODE COMMENTED BY YASH TO REMOVE TRANSPORT FEE FROM DAV BARIATU START
				...
				// if((($student_class_id==13)||($student_class_id==15)) && (($total_paid_month < 12) && ($total_pay_month >= 12 - $total_paid_month))) {          //March
				// 	$transport_fee_amt = $trans * ($total_pay_month - 1);
				// }
				
			// CODE COMMENTED BY YASH TO REMOVE TRANSPORT FEE FROM DAV BARIATU ENDS
			...
			case 35:
				...
				if((($student_class_id==10)||($student_class_id==12)) && (($total_paid_month < 12) && ($total_pay_month >= 12 - $total_paid_month))) {          //March
					$transport_fee_amt = $trans * ($total_pay_month - 1);
				}
				break; 
			...
			case 38:
				if(($total_paid_month < 1) && ($total_pay_month >= 1 - $total_paid_month)){ //April
					$transport_fee_amt = $student[0]->transport_amt * ($total_pay_month - 1);
				}
				break;
			...
			case 25:
				$trans	= $student[0]->transport_amt;
				$trans_a	= $student[0]->transport_amt;
                // echo '====';
                // echo $total_paid_month;
                // echo '====';
                // echo $total_pay_month;
				...
				//         $transport_fee_amt = $student[0]->transport_amt * ($total_pay_month - 2);
				// }
                
				if($total_paid_month != 1) {
                  // echo 'ed'.$total_paid_month;
                  // echo '<br>';
                  
					if(($total_paid_month < 2) && ($total_pay_month >= 2 - $total_paid_month)){ //May
                  // echo 'first sec';
						$transport_fee_amt =  $trans_a * ($total_pay_month - 2);
					}
                        
					if(($total_paid_month < 3) && ($total_pay_month >= 3 - $total_paid_month)){ //May
						$transport_fee_amt =  $trans_a * ($total_pay_month - 3);
					}
                        
					if(($total_paid_month < 1) && ($total_pay_month > 3) ){ //May
						$transport_fee_amt =  $student[0]->transport_amt * ($total_pay_month - 3);
					}
                        
					if(($total_paid_month == 1) && ($total_pay_month > 2) ){ //May
						$transport_fee_amt =  $student[0]->transport_amt * ($total_pay_month - 2);
					}
                        
					if(($total_paid_month == 2) && ($total_pay_month > 1) ){ //May
						$transport_fee_amt =  $student[0]->transport_amt * ($total_pay_month - 1);
					}
                    // else
                    // {
                    ...
                    //     }
                    // }
				} else if (($total_paid_month == 1) && ($total_pay_month == 1)) { //May
                          		$transport_fee_amt =  $trans_a * ($total_pay_month - 2);
					...
					if(($total_paid_month < 1) && ($total_pay_month >= 1 - $total_paid_month)) { //April
						$transport_fee_amt = $trans_a * ($total_pay_month - 1);
					}
                  
					if(($total_paid_month > 3)) { //May
						$transport_fee_amt =  $student[0]->transport_amt * ($total_pay_month - 3);
					}
				}
			// print_r($transport_fee_amt);die();
			// if((($student_class_id==13)||($student_class_id==15)) && (($total_paid_month < 12) && ($total_pay_month >= 12 - $total_paid_month))) {          //March
			//     $transport_fee_amt = $trans * ($total_pay_month - 1);
			// }
				break;
...
//-----------Late Fine --------------- //   
    ...
    //                    }
				...
				if(($this->session->userdata('school_id')==25) ||($this->session->userdata('school_id')==29)||($this->session->userdata('school_id')==33)) {
					// echo 'yes';
                	       } else {
...
//-------------------------------- //
		...
		foreach ($monthly_fee->result() as $obj) {
			...
			$table_html .= '<tr>';
			$table_html .= '<td style="white-space: nowrap">' . $obj->fee_name . '</td>';
			$table_html .= '<td style="white-space: nowrap">' . $obj->fee_amount * $total_pay_month .' </td>';
			$table_html .= '</tr>';
  ...
  //       die;
		...
		// }
		// else if($this->session->userdata('school_id')==33){
		// }
		// else{
			if ($student[0]->transport_amt != 0) {
...
//                                            $total = $total + $transport_fee_amt * $count;
		...
		if(($this->session->userdata('school_id')==25)||($this->session->userdata('school_id')==29)) {
		}
		else {
...
//        $pdf->Write(0, strtoupper($data['school_address']));
	...
	public function btn_download_pop_load() {
		...
		$query_transaction = $this->db->query("SELECT f1.*,f2.class_fee_head_id, "
											. " GROUP_CONCAT(DISTINCT(f2.fee_cat_id)) AS fee, "
											. " COUNT(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) AS m, "
											. " MIN(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) AS from_month, "
											. " MAX(f2.due_month_no) AS d "
											. " FROM fee_transaction_head f1, fee_transaction_det f2 "
											. " WHERE f1.id=$fee_transaction_id AND f1.response_message=0 AND f1.status=1 "
											. " AND f1.id=f2.fee_trans_head_id group by f1.id");
 //       $query_transaction = $this->db->query("select f1.*,f2.class_fee_head_id,group_concat(distinct(f2.fee_cat_id)) as fee,count(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) as m,min(case when f2.month_no<>0 and f2.fee_cat_id=2 then f2.month_no end) as from_month,max(f2.due_month_no) as d from fee_transaction_head f1, fee_transaction_det f2 where f1.id=$fee_transaction_id and f1.response_message=0 and f1.status=1 and f1.id=f2.fee_trans_head_id group by f1.id");
		...
		$student = $this->dbconnection->select('student', 'id, roll, course_id, transport_amt,'
									. ' class_id, (select class_name from class where id=student.class_id) as class_name,'
									. ' concat(first_name," ",middle_name," ",last_name) as name, admission_no, stud_category,'
									. ' (select c.cat_name from category c where c.id=stud_category) as cat_name,'
...
//Amit :: Code ends here 
            ...
            /*    $monthly_fee = $this->db->query("SELECT (" . $q[0]->m . "*fee_amount) AS fee_amount,"
													. " (SELECT fee_name from fee_master where id=class_fee_det.fee_id) AS fee_name "
													. " FROM class_fee_det "
													. " WHERE fee_cat=2 AND stud_cat=" . $student[0]->stud_category . " AND "
													. " class_fee_head_id=" . $q[0]->class_fee_head_id . " AND status=1"); */
...
//        }
	...
	public function btn_ereceipt_pop_load() {
		if ($this->uri->segment(3) == 'dwld_no')
			$fee_transaction_id = $this->input->post('trans_id');
		else
			$fee_transaction_id = $this->uri->segment(4);
//print_r($fee_transaction_id); die();
		$query_transaction = $this->db->query("SELECT * FROM store_transaction WHERE transaction_id= $fee_transaction_id AND paid_status=1");
		$q = $query_transaction->result();
		$school_id = $this->session->userdata('school_id');
		$school 	  = $this->school;
...
//        	$receipt_no = $q[0]->receipt_no;
		...
		$fee_type_name = 'Remarks';

		$fee_session_year = $this->academic_session[0]->fin_year;
		$session = $this->dbconnection->select('accedemic_session', '*', 'id=' . $q[0]->year);
		$accedemic_session = $session[0]->session;

		$total_amt = $q[0]->total_amount;
		$total_amt_words = $q[0]->total_amount;
		$data = array(
				'school_id' 	=> $school_id,
				'school_name'	=> $school[0]->description,
		...
		if ($this->uri->segment(3) == 'dwld_no')
			$this->load->view('feepayment/gateway/receipt_popup_store', $data);
		else
			$this->download_pdf($data);
	}
...
// overflow
    ...
    {
      $fee_id=$this->input->post('ido');

      $fetch_stock=$this->dbconnection->select('book_stock_master','*','fee_id='.$fee_id);
      //echo $this->db->last_query();die;
      echo $rest_qty=$fetch_stock[0]->rest_qty;
